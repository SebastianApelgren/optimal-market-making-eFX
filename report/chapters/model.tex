...
The dealer's inventory is described by a vector $Y(t) \in \R^d$, where $Y_i(t)$ represents the position in currency~$i$ valued in units of the reference currency. This accounting convention is central to the model. If the dealer holds $q_i$ units of currency~$i$, the reference-currency value of that position is $Y_i = q_i\,S_i$. Between trades, the quantity $q_i$ is constant, so the inventory inherits the dynamics of the exchange rate:
\begin{equation}
  \label{eq:inventory-mid}
  \dd Y_i\big|_{\text{mid}} = q_i \, \dd S_i = Y_i\!\left(\mu_i \, \dd t + \sigma_i \, \dd W_i\right).
\end{equation}
...
\end{equation}
For each unordered pair $\{i, j\}$, there are two possible trade directions: $(i,j)$ and $(j,i)$. The dealer quotes on both sides independently.
...
In the paper's numerical example, the arrival rates are the same for both directions of a given pair and for all tiers: $\lambda^n_k = \lambda_k$ depends only on the trade size and the pair. This direction symmetry implies that client buy and sell flow are balanced on average, so the optimal strategy has no systematic directional bias. The tiers differ only in their demand-curve parameters $\alpha_n$ and $\beta_n$.
...
The full model of \cite{barzykin2023} also includes parameters~$k_i$ that account for permanent market impact of the dealer's hedging activity on the mid-prices. These modify the effective hedging momentum by a small correction proportional to the inventory and the value function gradient. In the paper's calibration, $k_i \sim 10^{-7}$, which is negligible relative to the other terms. We omit these parameters in what follows and set $k_i = 0$ throughout.
...
We derive the PDE governing the value function using the dynamic programming principle. The idea is to consider the system's evolution over an infinitesimal time interval $[t, t + \dd t]$ and require that the value function be consistent with optimality. Three types of events can occur in this interval: exchange rates move continuously, a client trade may arrive and fill, and the dealer hedges continuously.
...
\begin{align*}
  \E\!\left[\dd\theta\big|_{\text{cont}}\right] = \Biggl(&\partial_t\theta + \frac{1}{2}\sum_{i,j=1}^d \Sigma_{ij}\,y_i\,y_j\,\frac{\partial^2\theta}{\partial y_i\,\partial y_j} + \sum_{i=1}^d \mu_i\,y_i\,\frac{\partial\theta}{\partial y_i}\\
...
Client trades arrive as Poisson jumps superimposed on the continuous dynamics. In $[t, t + \dd t]$, the probability that a request arrives on directed pair~$(i,j)$, tier~$n$, size~$z_k$ and the client fills is $\lambda^n_k\,f^n(\delta^n_{ij,k})\,\dd t$. A filled trade earns revenue $z_k\,\delta^n_{ij,k}$ and shifts the continuation value from $\theta(t, y)$ to $\theta(t, y + z_k\,d_{ij})$. The expected contribution from all quoting events in $[t, t+\dd t]$ is therefore
\[
  \sum_{(i,j),\,n,\,k} \lambda^n_k\,f^n\!\left(\delta^n_{ij,k}\right) \left[z_k\,\delta^n_{ij,k} + \theta(t, y + z_k\,d_{ij}) - \theta(t,y)\right] \dd t.
\]
Each summand combines the immediate profit from the markup with the change in future value from the inventory shift. The markup~$\delta$ controls the trade-off: a higher markup earns more per fill but reduces the fill probability.

\paragraph{Assembling the HJB equation.}
The dynamic programming principle requires that, under the optimal controls, the expected infinitesimal change in $\theta$ plus the flow payoff equals zero. Combining the continuous and jump contributions, subtracting the hedging costs and the running penalty, and requiring optimality, we obtain
\begin{align}
...
\end{align}
A crucial simplification now takes place. In the quoting supremum, the markup $\delta^n_{ij,k}$ for each combination of pair, tier, and size enters only its own summand, so the joint optimization separates into independent scalar problems. Rearranging the quoting contribution for a single event:
\[
...
\begin{equation}
  ...
  q_{ij} = \frac{\partial\theta}{\partial y_i} - \frac{\partial\theta}{\partial y_j},
\end{equation}
the difference in marginal values between the two currencies. The hedging Hamiltonian~\eqref{eq:hedging-hamiltonian} is precisely the Legendre--Fenchel transform of the execution cost~$L_{ij}$, a connection to convex duality that will yield the closed-form evaluation in Section~\ref{sec:model-hamiltonians}.
...
The Hamiltonians~\eqref{eq:quoting-hamiltonian} and~\eqref{eq:hedging-hamiltonian} are defined abstractly as suprema over controls. We now evaluate them in closed form for the logistic fill probability~\eqref{eq:logistic} and the execution cost~\eqref{eq:execution-cost}. These closed forms are used by both the ODE approximation (Chapter~\ref{ch:ode}) and the PDE solver (Chapter~\ref{ch:pde}).
