...
\begin{enumerate}
  ...
  \]
  This quantity is structurally zero in a two-currency model where GBP does not exist. It measures the cross-currency influence demonstrated in Figure~\ref{fig:ode-quotes}, where EURUSD quotes shift with GBP inventory through the off-diagonal entries of the Riccati matrix~$A$.
\end{enumerate}
...
\begin{enumerate}
  \setcounter{enumi}{3}
  \item \textbf{Own-pair hedge rate} (M\$/day). The optimal EURUSD hedge rate when holding 10\,M\$ in EUR:
  \[
...
QoIs~1, 2, and 4 have direct analogues in a two-currency setting. QoIs~3 and 5 probe mechanisms that exist only with three or more currencies. QoI~6 integrates across all pairs. This mix allows us to ask both whether the sensitivity conclusions from a single-pair analysis survive in the multi-currency model and what new dimensions the cross-currency coupling introduces.
...
\end{equation}
If $S_{T_i} \approx 0$, the parameter has no effect on the output and can be safely fixed at any value within its range. If $S_{T_i} \gg S_i$, the parameter participates in important interactions with other parameters. When the model is perfectly additive (no interactions), the first-order indices sum to one. Interactions cause $\sum_i S_i < 1$ and $\sum_i S_{T_i} > 1$, since the shared variance is counted once per participating parameter in the total-effect indices.
...
Before examining each QoI individually, Figure~\ref{fig:sa-heatmap} presents the total-effect Sobol indices for all 20 parameters and all 6 QoIs in a single heatmap. The block structure is immediately visible: the EUR/USD demand parameters (rows~6--9) light up only for QoI~1, the global market parameters (rows~1--5) dominate QoIs~2--5, and $\lambda_{\text{scale}}$ alone dominates QoI~6. The GBP/USD and EUR/GBP pair-specific parameters (rows~11--20) are dark across all QoIs. This pattern, which the following subsections examine in detail, reflects a clean separation between the microstructure parameters that control client pricing and the market-level parameters that control risk management.
...
The tier spread differential is controlled entirely by the EUR/USD demand curve parameters. The passive-tier logistic slope $\beta_2^{\text{EU}}$ accounts for 72\% of the output variance, followed by $\beta_1^{\text{EU}}$ (15\%), $\alpha_2^{\text{EU}}$ (9\%), and $\alpha_1^{\text{EU}}$ (3\%). All other parameters, including all global parameters and all GBP/USD and EUR/GBP parameters, have $S_{T_i} < 0.001$. The first-order indices sum to 1.03, indicating a perfectly additive model with no interactions.
...
The cross-inventory skew measures how GBP inventory shifts the EURUSD markup. This QoI probes a mechanism that does not exist in a two-currency model: the cross-currency coupling through the off-diagonal entries of~$A$. Figure~\ref{fig:sa-qoi3} shows the results.
...
The forward UQ confirms that the hedge rate is the least robust component of the strategy: mean $= -2{,}896$\,M\$/day, standard deviation $= 3{,}211$, CV $= 111\%$, 90\% CI $= [-9{,}287,\; 0]$\,M\$/day. The CI spans more than an order of magnitude. The distribution is heavily left-skewed because small values of~$\eta$ produce extremely large hedge rates through the $1/(2\eta)$ factor. The upper end of the CI reaches zero, meaning that for some parameter combinations the hedging momentum falls within the dead zone and the model prescribes no hedging at all.
...
\begin{figure}[ht]
  ...
  \caption{QoI~6: net revenue. Left: Sobol indices, dominated by $\lambda_{\text{scale}}$ (78\%). Right: output distribution, with 90\% CI entirely positive.}
  \label{fig:sa-qoi6}
\end{figure}

The arrival rate scale $\lambda_{\text{scale}}$ accounts for 78\% of the output variance. Revenue is roughly proportional to total client flow, and $\lambda_{\text{scale}}$ scales all arrival rates across all three pairs. The model is nearly additive ($\sum S_i = 0.99$, $\sum S_{T_i} = 1.05$). Secondary contributors include $\gamma$ (6\%), $\alpha_2^{\text{EU}}$ (4\%), $\sigma_{\text{EUR}}$ (3\%), and several demand parameters from multiple pairs at the 1--2\% level ($\alpha_2^{\text{GU}}$, $\beta_2^{\text{EU}}$, $\beta_2^{\text{GU}}$). This is the only QoI where GBP/USD demand parameters appear with nonzero Sobol indices, because net revenue sums contributions from all three pairs.

The forward UQ gives a mean of \$502k/day, CV $= 48\%$, and 90\% CI $= [\$124\text{k},\; \$893\text{k}]$/day. The CI is entirely positive: the strategy is profitable across every parameter combination in the sample. The main source of revenue uncertainty is simply how much client flow the desk receives, not the risk management parameters.
...
\midrule
Tier spread differential (QoI~1) & $\beta_2^{\text{EU}}$, $\beta_1^{\text{EU}}$, $\alpha_2^{\text{EU}}$, $\alpha_1^{\text{EU}}$ & None \\
Own-inventory skew (QoI~2) & $\sigma_{\text{EUR}}$, $\gamma$, $\lambda_{\text{scale}}$ & Small \\
Cross-inventory skew (QoI~3) & $\rho$, $\gamma$, $\lambda_{\text{scale}}$, $\sigma_{\text{GBP}}$ & Moderate \\
Own-pair hedge rate (QoI~4) & $\sigma_{\text{EUR}}$, $\gamma$, $\lambda_{\text{scale}}$, $\eta^{\text{EU}}$ & Significant \\
Cross-hedge momentum (QoI~5) & $\sigma_{\text{EUR}}$, $\rho$, $\gamma$, $\lambda_{\text{scale}}$ & Small \\
Net revenue (QoI~6) & $\lambda_{\text{scale}}$ & None \\
\bottomrule
\end{tabular}
\end{table}
...
This separation has practical implications for model calibration. A bank implementing the strategy of~\cite{barzykin2023} can calibrate the demand curve parameters from client fill data to control tiered pricing, and separately estimate the market parameters ($\sigma$, $\gamma$, $\lambda$) from broader market data to control the inventory management behavior. These two calibration tasks are largely independent and can proceed in parallel.

The sensitivity rankings for QoIs~1, 2, and~4 reproduce what a two-currency analysis (with 8 parameters) would give. Adding 12 new parameters and the cross-currency coupling mechanism does not change the conclusions for own-pair quantities. The multi-currency model introduces new sensitivity dimensions ($\rho$, $\sigma_{\text{GBP}}$) that appear only in the cross-pair QoIs, confirming that the additional complexity is justified when cross-pair effects are of interest.


% ------------------------------------------------------------------
\subsection{Parameters that can be fixed}
\label{sec:sa-fixable}

Of the 20 parameters studied, 10 have $S_{T_i} < 0.025$ across all six QoIs. These are all 10 pair-specific parameters for GBP/USD and EUR/GBP: the demand curves ($\alpha_1$, $\alpha_2$, $\beta_1$, $\beta_2$) and execution costs ($\eta$) for both pairs. They can be fixed at their nominal values without meaningfully affecting any output of the model.

The remaining 10 parameters form two groups. The five global parameters ($\sigma_{\text{EUR}}$, $\sigma_{\text{GBP}}$, $\rho$, $\gamma$, $\lambda_{\text{scale}}$) drive all risk management and cross-pair QoIs. The four EUR/USD demand parameters ($\alpha_1^{\text{EU}}$, $\alpha_2^{\text{EU}}$, $\beta_1^{\text{EU}}$, $\beta_2^{\text{EU}}$) plus $\eta^{\text{EU}}$ drive the quoting and hedging QoIs for the pair being quoted and hedged.

For a bank deploying this model, the message is concrete: calibration effort should focus on these 10 parameters. The demand curves of the non-primary pairs and their execution costs can be set approximately without affecting the strategy.
...
Own-pair hedge [M\$/day]       & $-2{,}896$ & 3{,}211 & $[-9{,}287,\; 0]$ & 111\% \\
...
\begin{figure}[ht]
  ...
  \caption{Scatter plot of the correlation~$\rho$ versus the cross-inventory skew ($n = 20{,}000$ samples). The cross-skew increases approximately linearly with~$\rho$, confirming that correlation gates the cross-pair coupling.}
