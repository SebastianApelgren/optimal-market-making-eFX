...
def optimal_delta_newton(p: float, alpha: float, beta: float) -> float:
...
def H_newton(p: float, alpha: float, beta: float) -> Tuple[float, float, float]:
...
def quadratic_coeffs_H_newton(alpha: float, beta: float,
                               eps_p: float = 1e-8) -> Tuple[float, float, float]:
    """Compute (alpha0, alpha1, alpha2) for H(p) around p=0 via Lambert W."""
    H0, delta0, f0 = H_newton(0.0, alpha, beta)
    alpha0 = H0
    alpha1 = -f0

    _, _, f_plus = H_newton(+eps_p, alpha, beta)
    _, _, f_minus = H_newton(-eps_p, alpha, beta)
    Hp_plus = -f_plus
    Hp_minus = -f_minus
    alpha2 = (Hp_plus - Hp_minus) / (2.0 * eps_p)
