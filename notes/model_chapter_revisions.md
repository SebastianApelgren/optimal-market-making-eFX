...
- [x] Rewrite eq:inventory-mid (Section 2.1) — replaced `|_mid` with `dY_i^{(price)}` superscript notation
- [x] Rewrite the hedging drift display (Section 2.3) — replaced `|_hedge` with `dY^{(hedge)}` superscript notation
- [x] Rewrite the Ito expansion (Section 2.5) — replaced `|_cont` with `dθ^{(cont)}` superscript notation
- [x] Added decomposition equation `dY = dY^{(price)} + dY^{(trade)} + dY^{(hedge)}` (eq:inventory-decomp) in Section 2.1
...
- [x] Rephrase arrival rate sentence for clarity
...
- [x] Kept k_i in the model (not set to zero). Added full momentum formula in eq:hedging-momentum and forward reference in Section 2.3
...
- [x] Kept as-is — clarifying sentence is sufficient, and the pedagogical approach differs from the paper's integral form
...
- [x] Replace "superimposed" → "alongside"
...
- [x] Replace "summand" → "term in the sum" (two instances)
...
- [x] Made brackets consistent — RHS changed from (...) to [...] to match LHS
...
- [x] Simplified — cut Legendre-Fenchel reference, now just "We evaluate in closed form in Section 2.6"
...
- [x] Simplified — "arises from applying Ito's formula to the inventory process"
...
- [x] Expanded DPP motivation (option a, rewritten for clarity around supremum consistency)
...
- [x] No change (confirmed consistent with PDE chapter)
...
- [x] Added n^d scaling explanation (no concrete example, just the exponential growth)
...
- [x] Rephrased — "used directly by PDE, indirectly by ODE via Taylor expansion"
...
- [x] Keep forward references as-is (no change)
- [x] Keep sensitivity foreshadowing as-is (no change)
- [x] Keep the Lambert W and hedging derivations in ch:model (mathematical content stays)
